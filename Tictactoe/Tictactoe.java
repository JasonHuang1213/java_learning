...
public class Tictactoe {
    ...
    private static void showBoard (int[][] gameboard) {
        int square = 1;	// starting from the first square
        final int cross = character('X');
        final int zero = character('0');
    
        for (int row = 0; row < gameboard.length; ++row) {		// gameboard.length - row number
            for (int col = 0; col < gameboard[row].length; ++col) {		// gameboard[row].length - column number
